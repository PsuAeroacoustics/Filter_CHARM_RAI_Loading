...
def import_charm_data(saved_params,res_params):
    ...
    saved_params.update({'loading':loading,'geometry':geometry})
...
def process_patch_geometry(saved_params,res_params):
    ...
    saved_params.update({'V':V,'patch_types':patch_types,'point_filt_ind':point_filt_ind,'A_patch':A_patch,'A_s':A_s,'patch_filt_ind':patch_filt_ind,'N_r_patch':N_r_patch,'N_phi_patch':N_phi_patch,'N_patches_tot':N_patches_tot,'N_patches':N_patches,'N_pnts_r':N_pnts_r,'N_pnts_phi':N_pnts_phi})


    # fig = plt.figure()
    # ax = fig.add_subplot(111, projection='3d')
    # ax.scatter(saved_params['geometry'].zones[0].nodes[0][np.invert(select_r_ind & select_phi_ind),0],saved_params['geometry'].zones[0].nodes[0][np.invert(select_r_ind & select_phi_ind),1],saved_params['geometry'].zones[0].nodes[0][np.invert(select_r_ind & select_phi_ind),2])
    # for i in range(N_patches_tot):
    #     ax.scatter(saved_params['geometry'].zones[0].nodes[0][(selected_ind[ind[i]]).flatten(),0],saved_params['geometry'].zones[0].nodes[0][(selected_ind[ind[i]]).flatten(),1],saved_params['geometry'].zones[0].nodes[0][(selected_ind[ind[i]]).flatten(),2])
...
def apply_treatment(saved_params,res_params):    
    ...
    for i in range(saved_params['N_patches']):
        ...
        saved_params['loading'].zones[0].data[:,filt_ind,1:] = apply_filt(saved_params['loading'].zones[0].data[:,filt_ind,1:],filt_resp[:,i])
...
def filter_rod_loads(saved_params,res_params):
    ...
    saved_params.update({'f':f,'dt':dt,'oaspl_baseline':oaspl_baseline})
    ...
    [saved_params.pop(key, None) for key in ['loading','geometry']]
