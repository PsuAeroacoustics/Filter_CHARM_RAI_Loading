...
exp_dir ='/Users/danielweitsman/Library/CloudStorage/OneDrive-ThePennsylvaniaStateUniversity/RES_BLADE_9_25/rotor_airframe_data/10_6/kde_t1775_iso_m90_r2'
charm_dir = "//Users/danielweitsman/Library/CloudStorage/OneDrive-ThePennsylvaniaStateUniversity/charm/rotor_af_interaction/quickISO_IAERO1_NOSWEEP/quickISO.1PSU-WOPWOP"
title =[r"$\mathrm{\phi = 23^\circ}$",r"$\mathrm{\phi = 0^\circ}$",r"$\mathrm{\phi = -23^\circ}$"]
leg_lab = [r'Measured',r'Predicted (VLM)']
class arg:pass
args = arg()
...
acs_data['geometry_values'] = np.flip(acs_data['geometry_values'],axis = 0)[args.mics]
acs_data['function_values'] = np.flip(acs_data['function_values'],axis = 0)[args.mics]
...
for mic_itr in range(len(args.mics)):
    ...
    ax.plot(acs_data['function_values'][mic_itr,0,:,0]/acs_data['function_values'][mic_itr,0,-1,0],np.roll(acs_data['function_values'][mic_itr,0,:,-1],-110),linestyle = linestyle[1],c = default_colors[1],label = 'Predicted',linewidth =2)
    ax.plot(t/t[-1],xn_records[mic_itr],linestyle = linestyle[0],c = 'grey',alpha = 0.05,zorder = 0)
    ax.legend(leg_lab,loc='lower right',fontsize=9,borderaxespad=0.25,handletextpad=0.5)
    ax.set(ylabel = r'p [Pa]', xlabel =r'Rev. Fraction',xlim = [0,1],ylim = [-1.5,1.5])
    ax.grid()
    plt.savefig(os.path.join(os.path.join(os.path.dirname(os.path.dirname(charm_dir)),f'p_tseries_m{args.mics[mic_itr]}_{os.path.basename(exp_dir)}_{os.path.basename(os.path.dirname(charm_dir))}.pdf')),format = 'pdf',bbox_inches='tight',pad_inches=.05)
    plt.close()
...
for mic_itr in range(len(args.mics)):
    ...
    ax[mic_itr,0].plot(acs_data['function_values'][mic_itr,0,:,0]/acs_data['function_values'][mic_itr,0,-1,0],np.roll(acs_data['function_values'][mic_itr,0,:,-1],-114),linestyle = linestyle[1],c = default_colors[1],label = 'Predicted',linewidth =2)
    ax[mic_itr,0].plot(t/t[-1],xn_records[mic_itr],linestyle = linestyle[0],c = 'grey',alpha = 0.05,zorder = 0)
    ax[mic_itr,0].set(xlim = [0,1],ylim = [-1,1],title =title[mic_itr])
